---
type: concept
created: 2026-06-15
updated: 2026-06-15
sources:
  - "[[sources/itu-t-l-1480-greening-by-digital]]"
tags:
  - "method"
aliases:
  - "Baseline scenario"
  - "Reference activity"
  - "Baseline scenario"
---

## Description
The reference scenario functions as the primary baseline for evaluating the environmental impact of digital interventions. It encompasses the reference activity, which is the non-ICT process that an ICT solution is intended to modify, optimize, or substitute. By modeling the carbon footprint of this activity in the absence of the digital solution, assessors can quantify the net enablement effect through comparative analysis. These scenarios must be projection-based, incorporating realistic technological and behavioral trends to ensure the validity of the assessment and maintain credibility in comparative environmental impact studies.

## Related Concepts
- [[concepts/greening-by-digital|Greening by Digital]]
- [[concepts/life-cycle-assessment|Life Cycle Assessment]]
- [[concepts/consequential-life-cycle-assessment|Consequential Life Cycle Assessment]]
- [[concepts/avoided-emissions|Avoided Emissions]]

## Related Entities
- [[entities/international-telecommunication-union|International Telecommunication Union]]

## Mentions in Source
- "The identification and definition of one or more reference scenarios is as important to the result of the assessment as the definition of the ICT solution scenario, and the most probable reference scenario shall be selected to serve as the baseline." — [[sources/itu-t-l-1480-greening-by-digital|ITU-T L.1480]]
- "The reference activity (e.g., by optimizing it or substituting it partially or entirely)." — [[sources/itu-t-l-1480-greening-by-digital|ITU-T L.1480]]
- "The reference scenario (the situation without the studied ICT solution applied, i.e., a situation with only the reference activity in place)." — [[sources/itu-t-l-1480-greening-by-digital|ITU-T L.1480]]